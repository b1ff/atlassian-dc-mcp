...
async function createTicket() {
  try {
    ...
    // Create the issue
    // Note: Issue type and priority IDs may need adjustment based on your Jira setup
    const issueParams = {
      projectId: 'DevOps',
    ...
    if (result.success && result.data) {
      ...
      const issueUrl = JIRA_API_BASE_PATH
        ? `${JIRA_API_BASE_PATH.replace('/rest', '')}/browse/${issue.key}`
      ...
      console.log('  - Reference to detailed reports in atlassian-dc-mcp/ directory');
    } else {
